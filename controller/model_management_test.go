// ABOUTME: Tests model metadata administration and pricing consistency.
// ABOUTME: Verifies atomic deletion and cache invalidation without changing channel routing.
package controller
		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			for _, rule := range []int{model.NameRuleExact, model.NameRulePrefix, model.NameRuleContains, model.NameRuleSuffix} {
				t.Run(fmt.Sprintf("rule_%d_exact_names_only_atomic_and_cached", rule), func(t *testing.T) {
					...
					cached, err := model.GetRandomSatisfiedChannel("default", name, nil, true, "", "")
					require.NoError(t, err)
					...
					for _, group := range []string{"default", "vip", "last-model-group"} {
						cached, _ = model.GetRandomSatisfiedChannel(group, name, nil, true, "", "")
						assert.Nil(t, cached)
					}
					cached, err = model.GetRandomSatisfiedChannel("default", name+"-keep", nil, true, "", "")
					require.NoError(t, err)
					require.NotNil(t, cached)
