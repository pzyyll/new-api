// ABOUTME: Builds scoped billing, routing, and conversion log metadata.
// ABOUTME: Preserves alpha soft-fail diagnostics and request User-Agent capture.
package service
...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	softFailInfo := make(map[string]interface{})
	AppendSoftFailAdminInfo(ctx, relayInfo, softFailInfo)
	other.MergeAdmin(softFailInfo)
	if ctx.Request != nil && ctx.Request.UserAgent() != "" {
		other.SetPublic("user_agent", ctx.Request.UserAgent())
	}
}
