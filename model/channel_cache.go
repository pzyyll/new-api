...
func GetRandomSatisfiedChannel(group string, model string, excludedChannelIds map[int]struct{}, allowMultiKeyFallback bool, userAgent string, requestPath string, filters ...dto.ChannelFilter) (*Channel, error) {
	// if memory cache is disabled, get channel directly from database
	if !common.MemoryCacheEnabled {
		return GetChannel(group, model, excludedChannelIds, allowMultiKeyFallback, userAgent, requestPath, filters...)
	}
	if requestPath != "" {
		filters = append(append([]dto.ChannelFilter(nil), filters...), dto.ChannelFilter{Kind: dto.FilterRequestPath, RequestPath: requestPath})
	}
