// ABOUTME: Tests plugin identity constraints during database channel selection.
// ABOUTME: Rejects mismatched generic plugins and permits compatible legacy providers.
package model
...
func TestTaskPluginChannelSelectionFiltersBothCachePaths(t *testing.T) {
	...
	selected, err := GetChannel("default", "shared", nil, true, "", "", identityFilters("alpha", nil)...)
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "alpha", selected.Name)
	selected, err = GetChannel("default", "shared", nil, true, "", "", identityFilters("", nil)...)
	require.NoError(t, err)
	assert.Nil(t, selected)
	selected, err = GetChannel("default", "ordinary", nil, true, "", "", identityFilters("", nil)...)
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "ordinary", selected.Name)
	selected, err = GetChannel("default", "legacy", nil, true, "", "", identityFilters("legacy-alpha", []int{constant.ChannelTypeKling})...)
	require.NoError(t, err)
	require.NotNil(t, selected)
	assert.Equal(t, "legacy-alpha", selected.Name)
	selected, err = GetChannel("default", "legacy", nil, true, "", "", identityFilters("legacy-alpha", []int{constant.ChannelTypeKling, constant.ChannelTypeJimeng})...)
	require.NoError(t, err)
	require.NotNil(t, selected)
